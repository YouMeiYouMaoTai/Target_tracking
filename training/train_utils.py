...
def get_annotations(annot_dir, sequence_dir, frame_file):
    ...
    Args:
        ...
        sequence_dir(str): The directory in which frame is located, relative to root directory
    ...
    Return:
        ...
        ## 包围框的 x轴、y轴 的 最左、最右、最上、最下 的坐标，可以确定一个矩形框

        width (int): The width of the jpeg image in pixels.
        height (int): The height of the jpeg image in pixels.
        valid_frame (bool): False if the target is present in the frame, and True otherwise.
        OBS: If the target is not in the frame the bounding box information are all None.
    """
    ...
    if isfile(annot_path):
        ...
        if root.find('object') is None:     ## 没找到
    ...
    valid_frame = True ## 有目标，则返回True；没有，就返回False
    return annotation, width, height, valid_frame
